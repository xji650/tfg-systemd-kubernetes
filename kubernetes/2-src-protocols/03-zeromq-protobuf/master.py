...
# =====================================================================
NODOS_FILLS = ["192.168.98.143", "192.168.98.144"] # Sin el :30000 aquí, lo montamos luego
...
def distribuir_modelo():
    print("\n" + "="*50)
    print("2. DISTRIBUYENDO EL MODELO VÍA ZEROMQ")
    print("="*50)
    
    with open(MODEL_PATH, "rb") as f:
        model_bytes = f.read()
        
    req = mnist_pb2.ModelRequest(model_data=model_bytes)
    payload_serialized = req.SerializeToString()
    ...
    for ip in NODOS_FILLS:
        ...
        try:
            ...
            res = mnist_pb2.ModelResponse()
            res.ParseFromString(resp_bytes)
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("3. INICIANDO BENCHMARK DE INFERENCIA (RED ZEROMQ)")
    print("="*50)
    ...
    for i, ip in enumerate(NODOS_FILLS):
        ...
        peticion = mnist_pb2.BatchRequest(batch_id=i, image_data=particion_np.tobytes())
    ...
    if exitos:
        ...
        try:
            ...
            fig.suptitle(f"10 Exemples aleatoris (ZeroMQ) - Worker [{exitos[0]['addr']}]", fontsize=16)
            for idx, ax in enumerate(axes.flat):
                ...
                ax.axis('off')
            plt.tight_layout()
            plt.savefig(os.path.join(ASSETS_DIR, 'ejemplos-predicciones-zmq.png'), bbox_inches='tight')
            plt.close()
        except Exception as e:
    ...
    print("\n" + "="*50)
    print(" RESULTADOS DE LA COMPARATIVA TÉCNICA (ZeroMQ) ")
    print("="*50)
    print(f"{'Protocolo de Comunicación:':<30} ZeroMQ (Protobuf)")
