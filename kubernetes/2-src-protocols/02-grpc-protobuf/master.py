...
# =====================================================================
NODOS_FILLS = ["192.168.98.143:30000", "192.168.98.144:30000"]
    ...
    for addr in NODOS_FILLS:
        try:
            with grpc.insecure_channel(addr, options=[
                ...
                res = stub.UploadModel(mnist_pb2.ModelRequest(model_data=model_bytes))
                print(f"Worker [{addr}] respondió: {res.message}")
...
def enviar_tarea_grpc(config):
    ...
    try:
        with grpc.insecure_channel(addr, options=[
            ...
            stub = mnist_pb2_grpc.MnistServiceStub(channel)
            req = mnist_pb2.BatchRequest(batch_id=batch_id, image_data=payload)
            res = stub.Procesar(req, timeout=300)
            
        fin_rtt = time.perf_counter()
        return {
            "addr": addr,
            "ram_max_mb": res.ram_usage,
            "cpu_promedio": res.cpu_usage,
            "t_proc_ms": res.t_proc_ms,
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("3. INICIANDO BENCHMARK DE INFERENCIA (RED gRPC)")
    print("="*50)
    ...
    for i, addr in enumerate(NODOS_FILLS):
        ...
        # MAGIA BINARIA gRPC: Comprimimos directamente a bytes C puros
    ...
    if exitos:
        ...
        try:
            ...
            fig.suptitle(f"10 Exemples aleatoris (gRPC) - Worker [{exitos[0]['addr']}]", fontsize=16)
            ...
            for idx, ax in enumerate(axes.flat):
                ...
                ax.set_title(f"Predicció: {pred}\nReal: {real}", color="green" if pred == real else "red")
                ax.axis('off')
            
            plt.tight_layout()
            plt.savefig(os.path.join(ASSETS_DIR, 'ejemplos-predicciones-grpc.png'), bbox_inches='tight')
            plt.close()
        except Exception as e:
    ...
    print("\n" + "="*50)
    print(" RESULTADOS DE LA COMPARATIVA TÉCNICA (gRPC) ")
    print("="*50)
    print(f"{'Protocolo de Comunicación:':<30} gRPC (Protobuf)")
