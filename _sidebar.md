- **> Setup**
  - [Configuración Nodos (MV)](Setmana0_setup-mv/Setmana0.md)
  
- **> Podman**
  - [Instalación Podman](Setmana1_podman/Setmana1.md)
  
- **> Systemd**
  - [Systemd Legacy](Setmana2_systemd-legacy/Setmana2.md)
  - [Systemd Quadlets](Setmana3.1_systemd-quadlets/Semana3.1.md)
  
- **> Kubernetes**
  - [Setup K3S](Setmana5_kubernetes/setup.md)
  
- **> Entregas**
  - **> Entrega 1**:
    - [Ansible + Systemd + Podman](entrega-1/entrega-1.md)
    - [README Entrega 1](entrega-1/readme.md)

  - **> Entrega 2**:
    - **> Resultados**
      - [Experiments HTTP](entrega-2/results/http/experimemts.md)
      - [Experiments gRPC](entrega-2/results/grpc/experimemts.md)
      - [Experiments ZeroMQ](entrega-2/results/zeromq/experimemts.md)
    - [Resultados HTTP](entrega-2/results/http/readme.md)
    - [Resultados gRPC](entrega-2/results/grpc/readme.md)
    - [Resultados ZeroMQ](entrega-2/results/zeromq/redme.md)
    - [Comparativa Systemd](entrega-2/entrega-2.md)
    - [README Entrega 2](entrega-2/readme.md)